...
namespace TechNova_IT_Solutions.Pages.ChiefComplianceManager
{
    public class ComplianceDashboardModel : PageModel
    {
        ...
        public int ApprovedImports { get; set; }
        ...
        public List<PolicyImportQueueItem> PolicyImportQueue { get; set; } = new();

        public class PolicyImportQueueItem
        {
        ...
        public async Task<IActionResult> OnGet()
        {
            ...
            // Load import queue stats
            PendingImports = await _context.ExternalPolicyImports
                .CountAsync(i => i.ReviewStatus == "PendingReview");
            ApprovedImports = await _context.ExternalPolicyImports
                .CountAsync(i => i.ReviewStatus == "Approved");

            // Load recent pending imports for the dashboard queue
            PolicyImportQueue = await _context.ExternalPolicyImports
                .AsNoTracking()
                .Where(i => i.ReviewStatus == "PendingReview")
                .OrderByDescending(i => i.ImportedAt)
                .Take(8)
                .Select(i => new PolicyImportQueueItem
                {
                    ImportId = i.ImportId,
                    PolicyTitle = i.PolicyTitle,
                    Category = i.Category ?? "General",
                    ReviewStatus = i.ReviewStatus ?? "PendingReview",
