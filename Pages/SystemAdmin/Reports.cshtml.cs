...
namespace TechNova_IT_Solutions.Pages.SystemAdmin
{
    public class ReportsModel : PageModel
    {
        ...
        public List<TechNova_IT_Solutions.Pages.ComplianceReportItem> ComplianceReportData { get; set; } = new();
        public List<TechNova_IT_Solutions.Pages.SupplierReportItem> SupplierReportData { get; set; } = new();
        public List<TechNova_IT_Solutions.Pages.ProcurementReportItem> ProcurementReportData { get; set; } = new();
        ...
        private IActionResult? RequireAuthorizedReportUser()
        {
            ...
            GeneratedByRole = userRole ?? RoleNames.SystemAdmin;

            // System Admin sees all reports (no branch scoping)
            return null;
        }
        ...
        private async Task LoadComplianceReportAsync()
        {
            ...
            ComplianceReportData = await _context.PolicyAssignments
                ...
                .Select(pa => new TechNova_IT_Solutions.Pages.ComplianceReportItem
                {
        ...
        private async Task LoadSupplierReportAsync()
        {
            ...
            SupplierReportData = await _context.SupplierPolicies
                ...
                .Select(sp => new TechNova_IT_Solutions.Pages.SupplierReportItem
                {
        ...
        private async Task LoadProcurementReportAsync()
        {
            ...
            ProcurementReportData = await _context.Procurements
                ...
                .Select(p => new TechNova_IT_Solutions.Pages.ProcurementReportItem
                {
        ...
        private string BuildCsv()
        {
            ...
            if (ReportType == "supplier")
            {
                ...
                foreach (var row in SupplierReportData)
                    builder.AppendLine(string.Join(",", EscapeCsv(row.SupplierName), EscapeCsv(row.AssignedPolicy), EscapeCsv(row.ComplianceStatus), EscapeCsv(row.AssignedDate?.ToString("yyyy-MM-dd") ?? "N/A")));
            }
            else if (ReportType == "procurement")
            {
                ...
                foreach (var row in ProcurementReportData)
                    builder.AppendLine(string.Join(",", EscapeCsv(row.ProcurementId), EscapeCsv(row.ItemName), EscapeCsv(row.Supplier), EscapeCsv(row.LinkedPolicy), EscapeCsv(row.PurchaseDate?.ToString("yyyy-MM-dd") ?? "N/A"), row.Quantity.ToString(), EscapeCsv(row.ApprovalStatus)));
            }
            else
            {
                ...
                foreach (var row in ComplianceReportData)
                    builder.AppendLine(string.Join(",", EscapeCsv(row.EmployeeName), EscapeCsv(row.AssignedPolicy), EscapeCsv(row.ComplianceStatus), EscapeCsv(row.AssignedDate?.ToString("yyyy-MM-dd") ?? "N/A"), EscapeCsv(row.AcknowledgedDate?.ToString("yyyy-MM-dd") ?? "N/A")));
            }
